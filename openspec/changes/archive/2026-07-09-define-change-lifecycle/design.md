...
- Drift note updated 2026-07-09: work item 1.8 reconciled `scripts/validate_change.py` to the canonical six lifecycle states and now rejects the historical vocabulary (`tasks_created`, `in_dev`, `ready_for_qa`, `implemented`) inherited from the removed architecture draft. This resolves tasks item 2.2, but validator or CI enforcement of lifecycle transitions remains a separate later decision under tasks item 2.3.
