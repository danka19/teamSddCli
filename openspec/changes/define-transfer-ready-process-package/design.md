...
- Produce a reusable release candidate that installs the deterministic thin-change process from a versioned package.
- Make configuration, validation, setup, upgrade, rollback, and transfer evidence reproducible.
- Prove the reference thin flow both without AI and with Qwen/DeepSeek-class assistants.
...
External release acceptance requires a clean bootstrap, package/config validation, reference thin-flow walkthrough, AI-disabled walkthrough, weak-model certification, negative safety cases, upgrade/rollback evidence, and a complete transfer manifest.

Corporate pilot acceptance begins only from an externally accepted release candidate. It verifies actual runtime, paths, registries, policies, integration wiring, adapter behavior, and one real thin change. Corporate evidence may identify compatibility gaps, but it does not silently redefine the reusable contract.
...
- [The process package grows into a custom CLI platform] -> Implement only the accepted thin flow and use scripts/standard tools until a strategy trigger is evidenced.
...
5. Prove clean bootstrap, reference thin flow, AI-disabled operation, actual weak-model runs, negative cases, and upgrade/rollback behavior.
...
8. Execute one monitored real thin-change pilot and record compatibility and usability evidence.
