...
def test_installed_launcher_metadata_and_version_diagnostics(capsys) -> None:
    ...
    assert payload == {
        ...
        "package": {"id": "sdd-process", "version": "0.3.8"},
        "schema_version": "1.0",
        "status": "ok",
    }
...
def test_installed_windows_or_posix_console_script_delegates_to_the_dispatcher(tmp_path: Path) -> None:
    ...
    assert json.loads(invoked.stdout)["package"] == {"id": "sdd-process", "version": "0.3.8"}
    ...
    assert all(token in help_output.stdout for token in ("setup", "start", "next", "0.3.8"))
