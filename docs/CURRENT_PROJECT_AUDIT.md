...
| Focused validator tests | `python -m pytest tests/test_validate_change.py -v` passed 13 tests after work item 1.8, covering thin/full artifact rules, canonical statuses, no-spec-change handling, waiver validation, traceability gaps, staged discovery, and placeholder mode; `pytest.ini` uses repository-local `.pytest-tmp` because the default Windows temp pytest directory is not accessible in this environment |
...
| AUDIT-016 | Work item 1.8 reconciled the deterministic layer with the approved Phase 1 contracts: `scripts/validate_change.py` now rejects the historical status vocabulary in favor of the canonical six states, keeps thin packages lightweight, enforces full-package-only artifacts and waivers where required, validates placeholder production values, preserves staged discovery behavior, and `templates/change/change.yaml` no longer defaults to the contradictory `mode: thin` + `type: new_feature` combination. See `docs/audits/FABLE5_DOCUMENTATION_ARCHITECTURE_REVIEW_2026-07-06.md` finding F1. | Phase 1 (work item 1.8) | closed |
