...
| AUDIT-017 | The OpenSpec CLI version is decided to be pinned, and `openspec 1.4.1` is the verified installed version. Draft proposal `openspec/changes/define-repo-topology-config/` now proposes pinning `1.4.1` in central `team-specs/sdd.config.yaml` with reviewed upgrade evidence, but the actual config file and binding pin location remain blocked until gate 1.5. | Phase 1 (work items 1.4/1.5) | open |
