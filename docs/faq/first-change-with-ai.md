...
отдельного согласия и готовит только один разрешённый текущим stage черновик.
После review черновика он задаёт новый вопрос:
...
Сначала AI показывает последовательность возможных factual artifact diffs —
по одному файлу за отдельное разрешение. Это не один пакетный edit: текущий
stage определяет, какой из следующих файлов разрешено предложить сейчас.
...
AI показывает только первый применимый diff. Следующий файл он предлагает лишь
после отдельного review и разрешения предыдущего; отказ или пауза не запускают
остальные edits.
...
Человек отдельно проверяет и разрешает каждый применимый artifact diff. Только
после этого AI предлагает отдельный diff для `change.yaml`, который добавляет
ровно 17 source-linked
...
```text
Разрешаю применить показанный classification_evidence diff только внутри этого
synthetic change. Decision block не менять.
```
