...
## Какой репозиторий перед вами

Этот репозиторий — исходный репозиторий самого `teamSddCli`: здесь
разрабатывают, проверяют и выпускают версионируемый process package
`sdd-process`. Он не является рабочим пространством команды, которая применяет
процесс к своим проектам. Исключение — изменения самого `teamSddCli`.

Чтобы использовать продукт в работе, команда проходит отдельный путь:

1. устанавливает принятую версию `sdd-process` из доверенного checkout или
   утверждённого package source и получает локальную команду `sdd`;
2. владелец процесса создаёт через `sdd setup` отдельное central workspace, где
   хранятся `team-specs`, configuration, changes, specs и evidence команды;
3. существующие code repositories остаются отдельными и при необходимости
   подключаются к central workspace через project adapter;
4. после настройки участники работают с установленной командой `sdd` и
   подготовленным workspace, а не разрабатывают framework в этом репозитории.

Ниже сначала объясняется назначение продукта, затем установка, setup и первый
учебный change. Точные команды установки и структура workspace вынесены в
отдельные страницы, чтобы не смешивать использование продукта с его
разработкой.
