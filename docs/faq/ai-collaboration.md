...
Это меню возможных следующих стадий, а не разрешение создать весь пакет сразу.
AI готовит только один черновик, разрешённый текущим stage; на intake действуют
более узкие правила role instruction.
...
### Если используется GigaCode

В workspace, подготовленном process package `0.3.8`, не нужно каждый раз
вставлять полный служебный prompt. GigaCode сначала читает package-managed
`Superpowers` — общие правила безопасной работы, — а затем
`sdd-process-companion`, который уточняет роль и текущий SDD-этап.

Поэтому человек может начать простой фразой: назвать свою роль и желаемый
результат. Но разрешение на конкретную команду или edit всё равно даётся
отдельно после показа exact action. Установка файлов и порядок их применения
описаны в [setup и topology](setup-and-topology.md).
