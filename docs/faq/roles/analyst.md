...
6. Для существующего change используйте:
   ...
   На real schema-v2 package сейчас ожидается известный
   `missing-lifecycle-state` blocker из-за mismatch с каноническим `status`.
   Не исправляйте metadata вручную; сохраните failure и используйте specialist
   lifecycle runbook.
