...
> Phase 1 has produced the first accepted OpenSpec baseline. Phase 2 is in progress: work items 2.1-2.13 and gates 2.14.1-2.14.3 are closed. Immutable candidate `phase-2-14-rc4` has its own manifest plus deterministic, AI-disabled, Qwen, DeepSeek, Windows, bounded native-WSL2, source-linked coverage, and passing review evidence. Its historical coverage remains `204 covered / 110 gaps / 20 future_work`. The 2026-07-20 selector review and focused-test remediation bind 62 Phase 2 selectors to exact existing or newly focused evidence, link 22 governance rows, route 12 Phase 3/4 rows to `future_work`, and leave 13 genuine product gaps. `D-019` confirms that the named deferred integrations do not block the first-MVP boundary. Six-group intake routes Delta semantics, archive history, and reviewed upgrades into apply-ready Phase 2 change `close-release-integrity-gaps`; seven later selectors stay visible for P3/P4. The working successor-source inventory remains `289/13/32` until implementation passes; this does not re-certify or mutate rc4. Explicit human acceptance is still required. Historical candidate `phase-2-12-rc7` and diagnostic rc2/rc3 remain preserved. No real corporate configuration or pilot has run, and macOS is not certified.
