...
> Phase 1 has produced the first accepted OpenSpec baseline. Phase 2 is in progress: work items 2.1-2.13 and historical rc4 gates 2.14.1-2.14.3 are closed. Immutable candidate `phase-2-14-rc4` remains historical at `204 covered / 110 gaps / 20 future_work`. The 2026-07-20 selector review, focused tests, and release-integrity remediation now bind 68 Phase 2 selectors to exact evidence, link 22 governance rows, route 12 Phase 3/4 rows to `future_work`, and leave seven explicitly deferred P3/P4 product gaps. `D-019` confirms that those deferred integrations do not block the first-MVP boundary. Working package `0.3.0` enforces Delta semantics, guarded archive history, and reviewed upgrades; working coverage is `295/7/32`. This source is not candidate-bound evidence and does not mutate rc4. Gate 2.14.4 remains blocked until independent review closes, a successor is frozen, and fresh deterministic, AI-disabled, Qwen/DeepSeek, Windows/WSL2, and review certification passes. Historical candidate `phase-2-12-rc7` and diagnostic rc2/rc3 remain preserved. No real corporate configuration or pilot has run, and macOS is not certified.
