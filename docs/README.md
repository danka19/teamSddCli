...
> Phase 1 has produced the first accepted OpenSpec baseline. Phase 2 is in progress: work items 2.1-2.5 closed the versioned foundation, deterministic classification/migration, class-aware artifact matrices, gate reports, and guarded lifecycle transitions; work item 2.6 is active for Tech Lead governance. Human decision `D-013` makes `minor | major | hotfix`, class-aware DoR/DoD, Tech Lead governance, corporate flow controls, and pilot evidence the target process. The release candidate must still implement and certify that target, AI-disabled operation, bootstrap/update/rollback, and transfer runbooks before corporate configuration and pilot work begin.
