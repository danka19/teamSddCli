...
- Capability-wide and bare-file evidence rules were removed. The product manifest contains one explicit row for every applicable effective selector, and a test-owned manifest independently binds each selector and stable binding ID to exact pytest node(s), certification case, or manual evidence. Missing rows, unrelated existing-node substitution, bare test files, and binding disagreement fail closed.
...
- Final worker coverage is 327 effective scenarios: 288 covered, 21 explicit allowed gaps, and 18 future-work scenarios. NIS confluence delta gaps are zero. Focused certification/feedback/package verification passed 64 tests with one environment-dependent Windows symlink-privilege skip; the full serial suite passed 407 tests with the same skip. Final governance/package/privacy/diff gates and commit are recorded in the worker handoff; work-item/task statuses remain unchanged for re-review.
