...
- Technical prerequisites and planning acceptance are complete. Work items 2.1-2.13 are closed; work item 2.14 is in progress, with 2.14.1-2.14.3 closed and 2.14.4 next.
...
Status: boundary and focused-test substeps are `closed`; product-gap intake is next. The bounded plan is `docs/planning/PHASE_2_FOCUSED_COVERAGE_TESTS_IMPLEMENTATION_PLAN_2026-07-20.md`.
```
...
| 2.14.4 Human acceptance | ready | The human owner receives immutable candidate rc4, manifest/checksums, platform and rollback evidence, AI-disabled evidence, Qwen/DeepSeek evidence, source-linked Tech Lead/QA/security evidence, limitations, the 2026-07-19 provenance audit, and the 2026-07-20 exact selector review. Rc4 still reports `204/110/20`; the improved `289/13/32` working source is successor preparation and is not certified. `D-019` accepts only the first-MVP exclusion boundary and does not start Phase 3. |
...
The AI-disabled, Qwen-family, and actual DeepSeek-family runtime/matrix execution from 2026-07-15 remains the immutable first baseline. Adapter `2.0` and `2.1` failures remain append-only evidence. Adapter `2.2` passed both family gates and AI-disabled 11/11; historical release candidate `phase-2-12-rc7` passed the accepted Windows/WSL2 contour, and the non-secret corporate-adaptation package passed deterministic closure/privacy/no-fork verification. Final candidate `phase-2-14-rc4` now has its own passing deterministic, AI-disabled, Qwen, DeepSeek, Windows, WSL2, source-linked coverage, and review evidence. Rc2 and rc3 remain immutable diagnostic review-failed history. Work items 2.1-2.13 and gates 2.14.1-2.14.3 are closed; mandatory human gate 2.14.4 is next. Prior passing `rc6` remains external release history.
