...
- Do not move reusable package, schema, validator, workflow, role-instruction, read-pack, certification, update/rollback, release-manifest, or guided-operation design into the corporate environment. Phase 2 completes and externally certifies the baseline core, Phase 3 completes the external guided-operation layer, and Phase 4 supplies only verified real configuration, approved wiring, thin adapters, and pilot evidence.
