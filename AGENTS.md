...
- Accepted corporate-process direction (2026-07-13, `D-013`): use the flat NIS-aligned target classification `minor | major | hotfix`; migrate supported legacy `thin -> minor` and `full -> major` without automatically inferring hotfix. Add class-aware DoR/DoD, separate release/archive/delivered states, Tech Lead governance and deterministic decision support, regression/scope/stop/escalation/release controls, role-understanding evidence, and outcome-based pilot measurement. AI remains advisory, hotfix cannot bypass mandatory safety or reconciliation, and PPRB organization/NIS project structure are not target architecture.
