...
- Accepted delivery strategy (2026-07-03): no custom `sdd` CLI upfront; the process is delivered via templates, validation scripts, pipelines, standard tool features, MCP-based Jira/Confluence access, and AI role skills. See `docs/IMPLEMENTATION_STRATEGY.md` for the triggers that justify building CLI parts.
- Environment constraint: reusable development happens externally with strong tooling and must produce an accepted transfer-ready release candidate certified with actual Qwen-class and DeepSeek-class assistants plus an AI-disabled walkthrough. The corporate environment may expose Qwen/DeepSeek/GigaCode-class assistants; it is limited to real configuration, approved integration wiring, thin adapter setup, environment checks, and a monitored pilot. Process guarantees must never depend on the AI layer.
...
- Accepted corporate-process direction (2026-07-13, `D-013`): use the flat NIS-aligned target classification `minor | major | hotfix`; migrate supported legacy `thin -> minor` and `full -> major` without automatically inferring hotfix. Add class-aware DoR/DoD, separate release/archive/delivered states, Tech Lead governance and deterministic decision support, regression/scope/stop/escalation/release controls, role-understanding evidence, and failed-run retention. AI remains advisory, hotfix cannot bypass mandatory safety or reconciliation, and PPRB organization/NIS project structure are not target architecture. Process-effectiveness measurement is not part of the target process.
- Accepted automation horizon (2026-07-14, `D-014`): deterministic and AI-disabled operation is the required foundation and permanent fallback, not the final AI-minimal product state. After the governed process and pilot are stable, later OpenSpec changes may progressively automate bounded workflow orchestration, drafting, evidence assembly, routing, monitoring, and permitted state-transition preparation with AI. Deterministic policy checks remain the verification/control plane, and human approvals, waivers, risk acceptance, merge, release, archive, and accountable ownership do not transfer to AI unless a future explicit human decision and accepted OpenSpec contract changes that boundary.
- Accepted reliability and throughput direction (2026-07-14, `D-016`): increase reliability through broader risk-oriented test coverage and end-to-end evidence traceability, and increase delivery speed through AI-assisted decomposition and parallel execution of explicitly independent tasks. Parallel work requires declared dependencies, owners, non-overlapping write scopes, separate evidence, and deterministic combined integration checks; it never weakens human authority or mandatory gates.
